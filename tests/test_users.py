...
def test_get_user_me(private_users_client: PrivateUsersClient, function_user: UserFixture):
    # Отправляем GET-запрос /api/v1/users/me на получение данных о "своём" пользователе
    response = private_users_client.get_user_me_api()

    # Инициализируем модель ответа на основе полученного JSON в ответе c проверкой его корректности
    response_data = GetUserResponseSchema.model_validate_json(response.text)

    # Проверяем статус-код ответа
    assert_status_code(response.status_code, HTTPStatus.OK)

    # Сравниваем тело ответа с данными ранее созданного пользователя
    assert_get_user_response(response_data, function_user.response)

    # Валидируем тело ответа на соответствие ожидаемой JSON-схеме
    validate_json_schema(response.json(), response_data.model_json_schema())
